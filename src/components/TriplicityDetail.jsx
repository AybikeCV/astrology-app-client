...
function TriplicityDetail() {
    ...
    const getTriplicityData = async() => {
        try{
            const response = await axios.get(`${import.meta.env.VITE_SERVER_URL}/triplicities/${triplicityId}?_embed=signs`)
 ...
 <h4>{triplicity.name}</h4>
                ...
                <p>Description: {triplicity.description}</p>

 <h5>Zodiac Signs That Belongs To This Triplicity: </h5>
{triplicity.signs.map((eachSign) => {
return (

<div> <Link to={`/signs/${eachSign.id}`}><img src={eachSign.imgUrl} style={{width:"200px"}}/></Link>
</div>
)
